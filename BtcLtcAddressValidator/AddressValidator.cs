...
namespace BtcLtcAddressValidator
{
        ...
        private static class Bech32
        {
            ...
            internal static byte[] Decode(string address)
            {
                if (address.Any(char.IsUpper) && address.Any(char.IsLower))
                    throw new FormatException("Error at Bech32 decoding, only all lowercase and all uppercase Bech32 addresses allowed");
            ...
            private static byte[] DecodeSegwitDataPart(byte[] base32Array)
            {
                ...
                if (destinationBitCountWithOutZeroBits - destinationBitCountWithZeroBits > 4)
                    throw new FormatException("Error at Bech32 decoding, incomplete last witness program byte has more than 4 bit");
                for (var i = 0; i < destinationBitCountWithOutZeroBits - destinationBitCountWithZeroBits; i++)
                {
                    if (bitArraySource[bitArraySource.Length - i]) throw new FormatException("Error at Bech32 decoding, incomplete last witness program byte not zero");
                }
                ...
                if (byteArray.Length < 2 || byteArray.Length > 40)
                    throw new FormatException("Error at Bech32 decoding, witness program not between 2 and 40 bytes long");
            ...
            private static byte[] ToBase32Array(string inputBase32)
            {
                ...
                for (var i = 0; i < inputBase32.Length; i++)
                {
                    ...
                    else throw new FormatException("Error at Bech32 decoding, invalid character in Bech32 address: " + inputBase32[i]);
                }
